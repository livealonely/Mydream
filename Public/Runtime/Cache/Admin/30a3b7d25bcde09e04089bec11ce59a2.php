...
$(function(){
	$('#system_menu_form_add_parentid').combotree({url:'<?php echo U('System/public_menuSelectTree');?>'}); 
...
function systemMenuAddDialogFormSubmit(){
	$.post('<?php echo U('System/menuAdd');?>', $("#system_menu_add_dialog_form").serialize(), function(res){
		if(!res.status){
...
<table>
	<tr>
		...
		<td><input id="system_menu_form_add_parentid" name="info[parentid]" class="easyui-combotree" value="<?php echo ((isset($_GET['parentid']) && ($_GET['parentid'] !== ""))?($_GET['parentid']):0); ?>" style="width:180px;height:26px" /></td>
