...
<!-- 添加角色 -->
<div id="admin_rolelist_add_dialog" class="easyui-dialog" title="添加部门" data-options="modal:true,closed:true,iconCls:'icons-application-application_add',buttons:[{text:'确定',iconCls:'icons-other-tick',handler:function(){$('#admin_rolelist_add_dialog_form').submit();}},{text:'取消',iconCls:'icons-arrow-cross',handler:function(){$('#admin_rolelist_add_dialog').dialog('close');}}]" style="width:480px;height:300px;"></div>
...
var member_rolelist_datagrid_toolbar = [
	{ text: '添加部门', iconCls: 'icons-arrow-add', handler: adminRoleListAdd },
...
function adminRoleListOrderFormatter(val, arr){
	return '<input class="admin_rolelist_order_input" type="text" name="order['+arr['m_roleid']+']" value="'+val+'" size="2" style="text-align:center" />';
}
//状态格式化
...
function adminRoleListRefresh(){
	$('#member_rolelist_datagrid').datagrid('reload');
}
//添加
function adminRoleListAdd(){
	$('#admin_rolelist_add_dialog').dialog({href:'<?php echo U('Member/roleAdd');?>'});
...
function adminRoleListEdit(id){
	...
	var url = '<?php echo U('Member/roleEdit');?>';
	url +='&id='+id;
...
function adminRoleListDelete(id){
	...
	$.messager.confirm('提示信息', '确定要删除吗？', function(result){
		...
		$.post('<?php echo U('Member/roleDelete');?>', {id: id}, function(res){
			if(!res.status){
